...
public class Unit {
	 ...
	 * 
	 * if ((this.getCounter() >= REST_INTERVAL) && (this.getStatus() !=
	 * "Resting")) { this.setCounter(0); this.rest(); return; }
	 * 
	 * this.setCounter(this.getCounter() + duration);
	 * 
	 * long durationM = (long) (1000 * duration); if (this.getStatus() ==
	 * "Fighting"){ try { wait(durationM); } catch (InterruptedException e) {
	 * e.printStackTrace(); } }
	 * 
	 * if (this.getStatus() == "Moving"){
	 * 
	 * double[] speed = this.getSpeed(); try { wait (durationM); double[] oldPos
	 * = this.getPosition(); double[] newPos = { oldPos[0] + (duration *
	 * speed[0]), oldPos[1] + (duration * speed[1]), oldPos[2] + (duration *
	 * speed[2]) }; if (this.isSprinting()) this.setStamina(this.getStamina() -
	 * 1); if (this.getStamina() == 0) this.stopSprinting(); if
	 * (isValidPosition(newPos)) this.setPosition(newPos);
	 * 
	 ...
	 * 
	 * 
	 * if (this.getStatus() == "Working"){ try { wait(durationM); } catch
	 * (InterruptedException e) { throw new InterruptedException(); }
	 * 
	 * }
	 * 
	 * if(this.getStatus()== "Resting"){ try { wait(durationM); } catch
	 * (InterruptedException e) { throw new InterruptedException(); }
	 * 
	 * } }
	 */
	...
	/**
	 * Pair two units to eachother as opponents.
	 * 
	 * @param opponent
	 *            The unit to set as apponent.
	...
	/**
	 * Advance a unit's game time and update it's position, attributes and
	 * status according to it's current status.
	 * 
	 * @param dt
	 *            The amount of time a unit will advance.
	 * @throws InterruptedException
	 * @throws IllegalArgumentException
	 *             Throws an IllegalArgumentException if the given dt is larger
	 *             than 0.2 .
	 */
	...
	public void updatePosition(double dt) {
		...
		double[] newPos = { oldPos[0] + dt * velocity[0], oldPos[1] + dt * velocity[1], oldPos[2] + dt * velocity[2] };
	...
	public void moveToAdjacent(int x, int y, int z) {
		if (this.canBeInterrupted("Moving")) {
			...
			double[] oldPos = this.getPosition();
			this.setNextPosition(new double[] { oldPos[0] + x + 0.5, oldPos[1] + y + 0.5, oldPos[2] + z + 0.5 });
		}
	}
